...
class PredictClient(object):
  ...
  def print_response(self, response):
    ...
    responseDict = {}
    for i in range(self.num_outputs):
        responseDict[output2_values[i]] = str(output1_values[i])
    return responseDict
...
class ImageClassificationPredictClient(PredictClient):
  ...
  def __init__(self, parser, num_output):
    ...
    self.parser.set_defaults(output_key1 = 'logits')
  ...
  def do_predict(self):
    ...
    responseDict = self.get_outputs(input_data)
    return responseDict
...
class TextClassificationPredictClient(PredictClient):
  ...
  def __init__(self, parser):
    ...
    self.parser.set_defaults(output_key1 = 'logits')
    self.parser.set_defaults(output_key2 = 'logits')
    self.ns = self.parser.parse_args()
    self.num_outputs = 6
  def do_predict(self):
    import re
